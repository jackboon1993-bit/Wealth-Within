...
export default async function handler(req, res) {
  ...
  // origin than wealth-within.vercel.app, so every call from the app
  // needs explicit CORS permission or the browser blocks it before the
  // request reaches this handler at all — same pattern already used in
  // api/categorize-transactions.js. This was missing entirely, which is
  // exactly what "Failed to fetch" means: not a server-side error this
  // handler ever got the chance to produce, but the browser refusing to
  // even send the real request. Must come before the method check
  // below, since browsers send a preflight OPTIONS request first for a
  // POST like this.
